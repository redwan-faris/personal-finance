...
<div class="space-y-6">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">Update the transaction category information.</p>
        </div>
            ...
            <form action="{{ route('transaction-categories.update', $transactionCategory) }}" method="POST">
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $transactionCategory->name) }}"
                    ...
                    <div>
                        ...
                        <select name="type" id="type" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm @error('type') border-red-300 @enderror">
                            <option value="expense" {{ old('type', $transactionCategory->type) == 'expense' ? 'selected' : '' }}>Expense</option>
                            <option value="income" {{ old('type', $transactionCategory->type) == 'income' ? 'selected' : '' }}>Income</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="color" name="color" id="color" value="{{ old('color', $transactionCategory->color ?? '#3B82F6') }}"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  ...
                                  placeholder="Enter category description">{{ old('description', $transactionCategory->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
